...
def server():
    ...
    while stop != 1:
        if start:
            ...
            cmd = 'openssl s_server -cert ' + certdir + 'server-cert.pem -brief -tls1_3 -key ' + certdir + 'server-key.pem -accept 8080 -Verify 1 -CAfile ' + certdir + 'ca.pem -no_resumption_on_reneg -num_tickets 0' + curvecmd + ciphercmd
            print(cmd)
...
def client():
    ...
    while stop != 1:
        if start:
            curvecmd = " -curves " + currcurve
            ciphercmd = " -ciphersuites " + currcipher
            cmd = 'echo "0000000000" | openssl s_client -brief -tls1_3 -connect 127.0.0.1:8080 -cert ' + certdir + 'cert.pem -key ' + certdir + 'key.pem -CAfile ' + certdir + 'ca.pem' + curvecmd + ciphercmd
            print(cmd)
            time.sleep(2)
            p = subprocess.Popen(cmd, stdout=subprocess.PIPE, shell=True, preexec_fn=os.setsid)
            time.sleep(3)
